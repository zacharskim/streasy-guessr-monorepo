...
export default function PlayPage() {
  ...
  return (
    <main className="flex flex-col items-center justify-center p-4 max-w-2xl mx-auto">
      <div className="w-full px-4 py-2 border-b bg-white shadow-sm flex flex-col sm:flex-row
  sm:items-center sm:justify-between gap-2 dark:bg-neutral-900 dark:border-neutral-700">
            ...
            {Array.from({ length: 5 }).map((_, index) => (
              <CarouselItem key={index}>
                <div className="w-full aspect-[4/3] relative bg-gray-200 dark:bg-neutral-700">
                  <Image src={getApartmentImageUrl(currentApartment, index)} alt={`Apartment photo ${index
                    + 1}`} fill className="object-cover rounded-md" />
                </div>
              </CarouselItem>
            ))}
          </CarouselContent>
      ...
      {!submitted ? (
        <div className="w-full max-w-2xl flex flex-col gap-4">
          <div className="w-full">
            <Slider min={500} max={10000} step={50} value={[guessValue]} onValueChange={(val) =>
              setGuessValue(val[0])} className="w-full" />
          </div>
          <div className="flex items-center justify-center gap-4 w-full">
            <input type="number" value={guessValue} onChange={(e) =>
              setGuessValue(Number(e.target.value))} min={500} max={10000} className="flex-1 max-w-xs border rounded px-3
  py-2 text-center bg-white text-black dark:bg-neutral-800 dark:text-white dark:border-neutral-600"
            />
            <button onClick={handleSubmitGuess} disabled={loading} className="bg-black text-white
  px-8 py-2 rounded hover:opacity-90 disabled:opacity-50 dark:bg-white dark:text-black
  dark:hover:opacity-80 whitespace-nowrap">
  ...
  dark:text-blue-400">{lastGuess.score}</p>
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Off by
                  ${lastGuess.difference} ({lastGuess.percentage_off}%)</p>
              </div>
              {currentRound < totalRounds ? (
                <button onClick={handleNextRound} className="bg-black text-white px-6 py-2 rounded
   hover:opacity-90 mt-4 w-full dark:bg-white dark:text-black dark:hover:opacity-80">
                  Next Apartment
                </button>
              ) : (
                <div className="text-center p-4 bg-green-50 dark:bg-green-900 rounded-lg w-full">
                  <p className="text-lg font-bold">Game Over!</p>
                  <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-2">Final
                    Score: {totalScore}</p>
                </div>
              )}
            </>
